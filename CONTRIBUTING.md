...
| 规则编号 | 规则名称 | 约束 | 说明 |
|:---|:---|:---|:---|
| **G-01** | 类数量硬上限 | L1 类总数 ≤ **25**（含抽象 + 具体） | 超过上限必须先合并或下放现有类，才能新增。由 `core_schema.json` 中 `maxItems: 25` 强制执行。当前 24/25。 |
...
| **G-05** | 关系密度控制 | 关系数量 ≤ 类数量 × **1.0** | 避免关系爆炸。当前 13 关系 / 24 类 ≈ 0.54，健康范围内。 |
