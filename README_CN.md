...
> Channel, Location, MarketSegment, Decision 等类已降级至 [`extensions/common/`](extensions/common/) L2 通用扩展。
...
> **本节面向需要创建 L2 行业与业务领域扩展 或 L3 企业定制层的开发者和架构师。**
> 以下流程以创建 L3 企业定制层为主要示例，L2 行业与业务领域扩展 的流程类似。
...
- **可选**同时继承其他 L2（如 `L2_common_enterprise_extension`）
...
- [ ] 确认所需 L2 Extension 及版本
- [ ] 阅读上级定义中的类/关系，理解可扩展的 parent 节点
- [ ] 记录 `compatible_core_version` 和 `compatible_extension_version`
...
# 文件头应声明 "$schema": "../../schema/extension_schema.json"

# 使用 Python jsonschema (需安装)
pip install jsonschema
python -c "
import json, jsonschema
schema = json.load(open('schema/extension_schema.json'))
data = json.load(open('extensions/your-industry/your_extension.json'))
...
**对于 L3 企业层：**
```bash
# L3 文件同样引用 extension_schema.json（结构兼容）
# "$schema": "../../schema/extension_schema.json"
```
...
"extends": [
  ...
  "L2_consulting_industry_extension_v1"          // L2 咨询行业 v1.1
]
```
...
| 行业 | 目录 | 类数量 | 关系数量 | 状态 |
|:---|:---|:---:|:---:|:---|
| 通用企业扩展 | [`extensions/common/`](extensions/common/) | 10 | 5 | ✅ v1.0.0 |
| 咨询行业 | [`extensions/consulting/`](extensions/consulting/) | 40+ | 34 | ✅ v1.1.0 |
| 奢侈品行业 | [`extensions/luxury-goods/`](extensions/luxury-goods/) | 38 | 14 | ✅ v2.0.0 |

**期待社区贡献更多行业与业务领域扩展！** 如金融、制造、零售、医疗、教育等。
