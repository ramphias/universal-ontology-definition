...
**典型案例**（已修正）：v1.0 中的 `Channel`、`MarketSegment`、`Location` 在 v2.0 中从 L1 移除，转由合适的 L2 扩展或属性建模，因为它们不满足"任意企业都必须建模此概念"的 L1 门槛。
...
**熵增路径**：
```
docs-site/l1-core/classes.md 描述 Risk = "不确定性事件"
l1-core/universal_ontology_v1.json definition = "可能影响目标的不确定事件"
l0-platform/owl-rdf/core_ontology.ttl skos:definition = "威胁或机会"
...
### 4.2 当前 L1 健康状态（v2.1.0 基准）

```
类数量:        24 / 25   (使用率 96% — 接近上限警戒线)
根类数量:       4 / 5    (健康)
最大继承深度:   3 / 4    (健康)
关系密度:      13 / 24 = 0.54  (远低于 1.0 — 健康)
...
```
JSON 文件 (l1-core/universal_ontology_v1.json) 是唯一语义权威

  l1-core/*.json          ← 权威来源
      │
      ├── docs-site/**/*.md     ← 必须与 JSON 保持一致
      ├── l0-platform/owl-rdf/*.ttl ← 由 json_to_owl.py 自动生成
      ├── l0-platform/graphql/*.graphql ← 由转换脚本生成
      └── l0-platform/sql/*.sql    ← 由转换脚本生成
```
...
| 位置 | 修改类型 | 责任人 |
|:---|:---|:---|
| `l1-core/universal_ontology_v1.json` 的 `definition` / `definition_en` | 语义权威 | PR 提交者 |
| `docs-site/l1-core/classes.md` 或 `docs-site/l2-extensions/*.md` | 人类可读说明 | PR 提交者 |
| `l0-platform/owl-rdf/core_ontology.ttl` | 机器可读 | 运行 `json_to_owl.py` |
...
所有修改 `l1-core/` 目录的 PR 描述中**必须附上**以下清单（不附则 Reviewer 拒绝）：
...
```
第一级（机器权威）: l1-core/*.json, l2-extensions/**/*.json, l3-enterprise/**/*.json, schema/*.json
...
| 文档文件 | 对应 JSON 内容 | 检查项 |
|:---|:---|:---|
| `docs-site/l1-core/classes.md` | `l1-core/universal_ontology_v1.json` `.classes[]` | 类数量、名称、层级、定义 |
| `docs-site/l1-core/relations.md` | `l1-core/universal_ontology_v1.json` `.relations[]` | 关系名、domain/range、基数 |
| `docs-site/l1-core/axioms.md` | `l1-core/universal_ontology_v1.json` `.axioms[]` | 公理 ID、类型、引用 |
...
- 新增 L1 类后，必须在同一 PR 内更新 `docs-site/l1-core/classes.md`
...
- [ ] 考虑是否有 L1 类可以下放至合适的 L2 行业扩展（`l2-extensions/<industry>/`）
...
*版本：v1.0 | 基于 Universal Ontology Definition v2.1.0*
