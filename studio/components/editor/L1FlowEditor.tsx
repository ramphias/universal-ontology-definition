...
export function L1FlowEditor({ initialData }: { initialData: any }) {
  ...
  const { attrCounts, axiomCounts } = useMemo(() =>
    computeBadgeCounts(initialData?.classes || [], initialData?.attributes || [], initialData?.axioms || []),
    [initialData]);

  // 1. Calculate base L1 Topology
  useEffect(() => {
    if (!initialData || !initialData.classes) return;
    const nodeIds = new Set<string>(initialData.classes.map((c: any) => c.id));

    const initialNodes: Node[] = initialData.classes.map((c: any) => ({
      id: c.id,
      type: 'custom',
      position: { x: 0, y: 0 },
      data: {
        label: c.id, label_zh: c.label_zh, abstract: c.abstract, fullData: c,
        attributeCount: attrCounts[c.id] || 0, axiomCount: axiomCounts[c.id] || 0,
      },
    }));
    ...
    initialData.classes.forEach((c: any) => {
      if (c.parent) {
        initialEdges.push({
          id: `e-${c.parent}-${c.id}`,
          source: c.parent,
          ...
          type: 'smoothstep',
          animated: true,
          style: { stroke: '#86BC25', strokeWidth: 1.5, opacity: 0.8 },
        });
      } else {
        initialEdges.push({
          id: `e-${ROOT_ID}-${c.id}`,
          ...
          type: 'smoothstep',
          animated: true,
          style: { stroke: '#555', strokeWidth: 1.5, strokeDasharray: '4,4', opacity: 0.5 },
        });
      }
    });

    // Add relation + axiom edges when toggle is on
    const relationEdges = showRelations ? buildRelationEdges(initialData.relations || [], nodeIds) : [];
    const axiomEdges = showRelations ? buildAxiomEdges(initialData.axioms || [], nodeIds) : [];

    setRawNodes(initialNodes);
    setRawEdges([...initialEdges, ...relationEdges, ...axiomEdges]);
  }, [initialData, showRelations, attrCounts, axiomCounts]);
  ...
  // Dim relation edges not connected to selected node
          ...
          >
            <Panel position="top-left" className="z-50 ml-2 mt-2">
               <button 
                  ...
                  className="bg-[#111] hover:bg-black border border-[#333] text-deloitte-green hover:text-white px-3 py-1.5 rounded-lg text-sm flex items-center gap-2 shadow-xl transition-all hover:scale-105"
               >
            ...
            <Panel position="bottom-center" className="mb-[15px] z-50">
               <div className="flex items-center gap-2">
                 ...
                 <button
                    ...
                    className={`px-3 py-1 text-xs rounded-lg border shadow-xl transition-all ${
                      showRelations
                      ? 'bg-[#5C6BC0]/20 text-[#7986CB] border-[#5C6BC0]/50 shadow-[0_0_10px_rgba(92,107,192,0.2)]'
                      : 'bg-[#111] text-[#666] border-[#333] hover:text-[#AAA]'
                    }`}
                 >
                    ⇌ Relations
                 </button>
            ...
            <MiniMap
              nodeColor={(n) => n.data?.abstract ? '#333' : '#86BC25'}
